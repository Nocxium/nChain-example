...
while true; do
	...
	while read -r playing position length name artist title arturl hpos hlen; do
		...
		case $playing in
		⏸️ | Paused) text='<span foreground=\"#f4bde6\" size=\"smaller\">'" ""$line"'</span>' ;;
		▶️ | Playing) text='<span foreground=\"#c8a1f5\" size=\"smaller\">'" ""$line"'</span>' ;;
		*) text='<span foreground=\"#3c485d\"></span>' ;;
		esac
