...
while true; do
	...
	while read -r playing position length name artist title arturl hpos hlen; do
		...
		case $playing in
		⏸️ | Paused) text='<span foreground=\"#643936\" size=\"smaller\">'" ""$line"'</span>' ;;
		▶️ | Playing) text='<span foreground=\"#f2c97d\" size=\"smaller\">'" ""$line"'</span>' ;;
		*) text='<span foreground=\"#643936\"></span>' ;;
		esac
